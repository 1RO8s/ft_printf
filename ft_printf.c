...
/*   Updated: 2023/07/06 07:48:53 by hnagasak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	printf("printf pointer %p", (void *)ptr);
	return (1);
}

char	ft_get_hex_base(unsigned int n)
{
	if (0 <= n && n < 9)
		return ('0' + n);
	else if (10 <= n && n < 16)
...
char	*ft_get_hex_str(char *str, unsigned int n, int len)
{
	if (0 <= n && n < 16)
		*str = ft_get_hex_base(n);
	else if (16 <= n)
	{
		ft_get_hex_str(str, n / 16, len - 1);
		*(str + len - 1) = ft_get_hex_base(n % 16);
	}
	return (str);
}
...
int	ft_puthex(unsigned int num)
{
	...
	if (!str)
		return (-1);
	str = ft_get_hex_str(str, num, len);
	ft_putstr(str);
	free(str);
	return (len);
}
